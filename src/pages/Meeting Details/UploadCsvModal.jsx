...
console.log(data2)
  const {email,firstName,lastName,phoneNumber,sessionMinutes,csvName} = data2
...
const mergeDataByEmail = (data) => {
  ...
  data.forEach(item => {
      ...
      if (!mergedData[email]) {
          ...
          existing.totalTimeInSession += parseInt(item[sessionMinutesField], 10) || 0;
      }
  });
...
setModal(false)
  ...
  >
    ...
    {!mapUI && <div className='px-8 space-y-5'> 
 { !showFileUpload &&  <><p className='text-sm font-medium text-red-500'> Note : For zoom downloaded csv it starts with 14th line </p> 
 <div className='flex justify-center items-center gap-3  '>
    <label className="font-medium text-sm">Header Row Starts with :</label>
                  <input
                    type="number"
                    ...
                    required={true}
                  />
                  <button className='bg-blue-700 hover:bg-blue-800 text-white py-2 px-4 rounded-md text-center' onClick={handleNextClick}>Next </button>

                  </div><div>
                  {errorMessage && <span className="text-white bg-red-500 rounded-lg px-2 py-1 text-sm">{errorMessage}</span>}</div> </> }
     { showFileUpload && <> <div className="max-w-md h-40 rounded-lg border-2 border-dashed flex items-center justify-center">
            <label htmlFor="file" className="cursor-pointer text-center p-4 md:p-8">
               ...
               <svg className="w-10 h-10 mx-auto" viewBox="0 0 41 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <p className="mt-3 text-gray-700 max-w-xs mx-auto"> <span  className="font-medium text-indigo-600">Select a csv file</span> or drag and drop a csv file here</p>
            </label>
            <input id="file" accept=".csv" type="file" className="hidden" onChange={handleFileUpload} />
        </div>
 ...
 {mapUI && <form className='mt-2 px-10' onSubmit={handleSubmit(onSubmit)}>
            ...
            <td className='px-6 pb-2 '>  <Controller 
                                      ...
                                      render={({ field }) => (
                                          <Select
                                              ...
                                              onChange={(selectedOption) => {
                                                field.onChange(selectedOption);
                                                handleSelectChange('phoneNumber', selectedOption);
                                              }}
                                              className="mt-2"
                                              placeholder="Select a custom field"
            ...
            <td className='px-6 pb-2 '>  <Controller 
                                      ...
                                      render={({ field }) => (
                                          <Select
                                              ...
                                              onChange={(selectedOption) => {
                                                field.onChange(selectedOption);
                                                handleSelectChange('sessionMinutes', selectedOption);
                                              }}
                                              className="mt-2"
                                              placeholder="Select a custom field"
        ...
        <button onClick={dateError} className='bg-blue-700 hover:bg-blue-800 w-full text-white p-2 rounded-md text-center mt-7 mb-3'>Submit </button>
